...
def final_output(glob_name,glob_verb,glob_noun,glob_date,line_no):
    ...
    line.append(glob_date)
    relations[line_no] = line



def get_temporal_sentence_relations(ans):
    ...
    for i in range(len(ans['sentences'])):
        ...
        for props in ans['sentences'][i]['tokens']:
            if props['ner'] in ['DATE','TIME','DURATION','SET']:
                found = 1
                glob_date.append(str(props['word']))
            if props['ner'] == 'PERSON':
                if cont == 1:
                    name += ' ' + props['word']
                else:
        ...
        if found == 1:
            # print ans['sentences'][i]
            # raw_input()
            final_output(glob_name,glob_verb,glob_noun,glob_date,i+1)
    return relations
